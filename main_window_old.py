...
from PyQt6.QtWidgets import QMainWindow, QApplication, QTableView, QFileDialog, QMessageBox, QWidget, QDateEdit, \
    QVBoxLayout, QDialog, QCalendarWidget, QDialogButtonBox, QSpinBox, QLabel
...
KAKAO_TALK_EMPTY_DATAFRAME = pd.DataFrame(
    columns=['이름', '휴대폰번호', '정보1', '정보2']
)
...
PDF_TO_POSTMOA_NORMAL_MAIL_EXCEL_COLUMNS: Sequence[ColumnReplacer] = (
    ColumnReplacer('수취인*', '{name}'),
    ColumnReplacer('우편번호*', '{zipcode}'),
    ColumnReplacer('기본주소*', '{address}'),
    ColumnReplacer('문서제목', '{title}'),
    ColumnReplacer('비고', '{bike_number} {due_date}까지'),
...
PDF_TO_WINDOWED_ENVELOPE_COLUMNS: Sequence[ColumnReplacer] = (
    ...
    ColumnReplacer('기본주소', '{address}'),
    ColumnReplacer('수취인', '{name}'),
    ColumnReplacer('문서본문', '{bike_number}, {due_date}까지'),
)

COLUMNS_KR_TO_EN = {v: k for k, v in COLUMNS_EN_TO_KR.items()}

DATA_EMPTY_FRAME = pd.DataFrame(columns=list(COLUMNS_EN_TO_KR.keys()))
...
def extract_pattern_from_pdf(pdf: pathlib.Path | str, pattern: re.Pattern):
...
class ExcelMixin:
    ...
    def save_to_postmoa_mail_excel_new(self, data_df: pd.DataFrame, target: pathlib.Path | str,
                                       columns: Sequence[ColumnReplacer],
                                       postmoa_df: pd.DataFrame):
        if any(data_df):
            # replacer가 있으면 replacer가 적용된 text 입력
            for column in columns:
                if column.replacer:
                    column.replace(postmoa_df, data_df)

            postmoa_df.to_excel(target, index=False)
...
class ReportLabMixin:
    ...
    def save_to_windowed_envelope_order_address_only_pdf(self, data_df: pd.DataFrame, target: pathlib.Path | str,
                                                         columns: Sequence[ColumnReplacer],
                                                         windowed_envelope_df: pd.DataFrame):
        ...
        if any(data_df):
            ...
            for column in columns:
                if column.replacer:
                    column.replace(windowed_envelope_df, data_df)

        target = pathlib.Path(target)
        windowed_envelope_pdf = Canvas(filename=str(target), pagesize=A4)

        for row in windowed_envelope_df.itertuples():
            index, title, zipcode, address, name, body = row

            # print(index, title, zipcode, address, name, body)
            # 주소
...
class MainWindow(QMainWindow, ReportLabMixin, ExcelMixin):
    def __init__(self):
        super().__init__()

        self.setWindowTitle("Bike Inspection File to Postmoa Converter")
        ...
        self.data: pd.DataFrame = DATA_EMPTY_FRAME.copy(deep=True)
        self.model = None
        ...
        save_to_postmoa_action = QAction('Save to Mail', self)
        file_menu.addAction(save_to_postmoa_action)

        save_to_postmoa_action.setShortcut('Ctrl+M')
        ...
        ## save to action 추가
        save_to_pdf_action = QAction('Save to PDF', self)
        file_menu.addAction(save_to_pdf_action)

        save_to_pdf_action.setShortcut('Ctrl+P')
        save_to_pdf_action.setStatusTip('Save to PDF')
        save_to_pdf_action.triggered.connect(self.save_to_windowed_envelope_order_address_only_pdf)
    ...
    def save_to_postmoa_dialog(self):
        ...
        self.save_to_postmoa_mail_excel_new(self.data, save_to_postmoa_normal_mail_path,
                                            PDF_TO_POSTMOA_NORMAL_MAIL_EXCEL_COLUMNS,
                                            NORMAL_MAIL_EMPTY_DATAFRAME.copy(deep=True))
        self.save_to_postmoa_mail_excel_new(self.data, save_to_postmoa_registered_mail_path,
                                            PDF_TO_POSTMOA_REGISTERED_MAIL_EXCEL_COLUMNS,
                                            REGISTERED_MAIL_EMPTY_DATAFRAME.copy(deep=True))
        self.save_to_postmoa_mail_excel_new(self.data, save_to_postmoa_selective_registered_mail_path,
                                            PDF_TO_POSTMOA_SELECTIVE_REGISTERED_MAIL_EXCEL_COLUMNS,
                                            SELECTIVE_REGISTERED_MAIL_EMPTY_DATAFRAME.copy(deep=True))

    def save_to_address_only_pdf_dialog(self):
        ...
        directory = pathlib.Path(directory)
        save_to_windowed_envelop_pdf_path = directory / '{datetime}_창봉투_address_only.pdf'.format(
            datetime=arrow.now().format('YYYY-MM-DD HHmmss'))

        print('save_to_windowed_envelope_order_address_only_pdf called')

        self.save_to_windowed_envelope_order_address_only_pdf(self.data, save_to_windowed_envelop_pdf_path,
                                                              PDF_TO_POSTMOA_NORMAL_MAIL_EXCEL_COLUMNS,
                                                              DATA_EMPTY_FRAME.copy(deep=True))

    def clear_table(self):
        self.data = DATA_EMPTY_FRAME.copy(deep=True)
